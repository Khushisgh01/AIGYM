...
def sync_metrics_update(context):
    ...
    # Track whether a priority audio event fired this cycle so
    # ongoing_form_check cannot overwrite it.
    priority_audio_set = False
    ...
    # Only run ongoing_form_check if no higher-priority audio was set this cycle
    if not priority_audio_set and st.session_state.get("voice_pipeline"):
        result = st.session_state.voice_pipeline.process_event(
            event="ongoing_form_check",
            exercise=exercise,
            metrics=latest_metrics,
